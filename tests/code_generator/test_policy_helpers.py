...
def test_tf_helper_inserts_flatten_when_allowed():
    ...
    shape = (None, 8, 8, 3)
    insert, new_shape = _policy_ensure_2d_before_dense_tf(
        rank_non_batch=3,
...
def test_tf_helper_raises_when_not_allowed():
    ...
    with pytest.raises(ValueError):
        _policy_ensure_2d_before_dense_tf(3, False, propagator, (None, 8, 8, 3))
...
def test_pt_helper_appends_view_when_allowed():
    ...
    body = []
    shape = (None, 8, 8, 3)
    new_shape = _policy_ensure_2d_before_dense_pt(
        rank_non_batch=3,
        auto_flatten_output=True,
