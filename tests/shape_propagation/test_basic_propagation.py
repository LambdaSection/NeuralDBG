...
#########################################
# 18. Conv2D with kernel size exceeding input dimensions (using 'valid' padding)
#########################################
def test_conv2d_kernel_too_large():
    ...
    # The shape propagator should raise a ValueError when kernel size exceeds input dimensions
    with pytest.raises(ValueError) as excinfo:
        propagator.propagate(input_shape, layer, framework="tensorflow")
    assert "kernel size" in str(excinfo.value).lower()
    assert "exceeds input dimensions" in str(excinfo.value).lower()
